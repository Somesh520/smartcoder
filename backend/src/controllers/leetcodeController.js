...
export const getMe = async (req, res) => {
    try {
        ...
        // Save to DB
        if (req.user) {
            req.user.leetcodeUsername = username;
            await req.user.save();
        }
    ...
    try {
        ...
        const matches = allProblems
            .filter(p => {
                ...
                const isPaid = p.paid_only === true || p.paid === true || p.stat?.is_new_question === true;
...
export const getSubmissions = async (req, res) => {
    try {
        const username = req.params.username || req.user?.leetcodeUsername;
