  ...
  try {
    ...
    if (token) {
      try {
        ...
        console.error(
          "Invalid token passed to github auth redirect:",
          e.message,
        );
      }
    }
...
export const githubcallback = async (req, res) => {
  ...
  try {
    ...
    if (response.status !== 200)
      return res.status(400).json({ message: "Failed to fetch github auth" });
    ...
    if (state) {
      ...
      if (user) {
        ...
        console.warn(
          `User with ID ${state} not found during GitHub OAuth callback`,
        );
      }
    } else {
    ...
    res.redirect(
      `${process.env.CLIENT_URL || "http://localhost:5173"}/app/github?just_authorized=true`,
    );
  } catch (error) {
    console.error(error);
