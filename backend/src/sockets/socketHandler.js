  ...
  io.on("connection", async (socket) => {
    ...
    socket.on(
      ...
      async ({
        ...
        console.log(
          `[JoinRoom] Request for ${roomId} by ${username}. SpecificProblem: "${specificProblem}"`,
        );
        ...
        if (specificProblem) {
          console.log(
            `[JoinRoom] Setting specificProblem for room ${roomId}: ${specificProblem}`,
          );
    ...
    socket.on("startGame", async ({ roomId }) => {
      ...
      io.to(roomId).emit("gameStart", {
        message: room.specificProblem
          ? "Starting Custom Challenge..."
          : "Host started the battle! Starting in 5 seconds...",
      });

      // Fetch Random Problem
      try {
        let allProblems = await leetcodeService.fetchProblems();
        if (allProblems.stat_status_pairs)
        ...
        if (room.specificProblem) {
          ...
          console.log(
            `[StartGame] Searching for specific problem: "${targetSlug}" in ${allProblems.length} problems`,
          );
          ...
          const specificMatch = allProblems.find((p) => {
            ...
            const pId = String(
              p.id || p.questionId || p.stat?.question_id || "",
            );
          ...
          if (specificMatch) {
            console.log(
              `[StartGame] Match Found in Cache! ID: ${specificMatch.id || specificMatch.stat?.question_id}`,
            );

            const isPaid =
              specificMatch.paid_only === true || specificMatch.paid === true;
            if (!isPaid) {
              const pSlug =
                specificMatch.slug ||
                specificMatch.titleSlug ||
                specificMatch.stat?.question__title_slug;
              const pTitle =
                specificMatch.title || specificMatch.stat?.question__title;
              const pId =
                specificMatch.id ||
                specificMatch.questionId ||
                specificMatch.stat?.question_id;
              ...
              console.warn(
                `[GameStart] Specific problem is PAID. Falling back.`,
              );
              io.to(roomId).emit("chatMessage", {
                username: "System",
                message:
                  "⚠️ Custom problem is Premium/Locked. Using random instead.",
              });
            }
          } else {
            // FALLBACK: Fetch Individual Problem Details
            console.warn(
              `[StartGame] Problem NOT found in cache (${allProblems.length} items). Fetching direct details for: ${targetSlug}`,
            );

            try {
              const details =
                await leetcodeService.fetchProblemDetails(targetSlug);
              ...
              if (q) {
                if (q.isPaidOnly) {
                  io.to(roomId).emit("chatMessage", {
                    username: "System",
                    message: "⚠️ Custom problem is Premium. Using random.",
                  });
                } else {
              ...
              console.error(
                `[StartGame] Direct fetch failed for ${targetSlug}:`,
                err.message,
              );
              io.to(roomId).emit("chatMessage", {
                username: "System",
                message: `⚠️ Problem "${targetSlug}" not found. Using random.`,
              });
            }
          }
        } else {
        ...
        if (filtered.length === 0) {
          ...
          filtered = allProblems.filter(
            (p) => p.difficulty.level === diffLevel && !p.paid_only,
          );
        }

        if (filtered.length === 0) {
          filtered = allProblems.filter(
            (p) => p.difficulty.level === 2 && !p.paid_only,
          );
        }
        ...
        if (!randomProb) {
          ...
          let localFiltered = FALLBACK_PROBLEMS.filter(
            (p) => p.difficulty.level === diffLevel,
          );
          if (localFiltered.length === 0) localFiltered = FALLBACK_PROBLEMS;

          const pick =
            localFiltered[Math.floor(Math.random() * localFiltered.length)];
          room.problem = { id: pick.id, title: pick.title, slug: pick.slug };
        } else {
          room.problem = {
            id:
              randomProb.id || (randomProb.stat && randomProb.stat.question_id),
            title:
              randomProb.title ||
              (randomProb.stat && randomProb.stat.question__title),
            slug:
              randomProb.title_slug ||
              (randomProb.stat && randomProb.stat.question__title_slug),
          };
        }
    ...
    socket.on("killSession", ({ userId }) => {
      ...
      if (existingSession) {
        ...
        if (result) {
          ...
          const wasActive =
            room && (room.status === "active" || room.status === "starting");
          if (wasActive) {
        ...
        socket.emit("sessionKilled", {
          message: "Previous session ended. Re-joining...",
        });
      } else {
        socket.emit("sessionKilled", {
          message: "No active session found. Retrying...",
        });
      }
    });
