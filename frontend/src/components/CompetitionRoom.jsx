...
const CompetitionRoom = ({ socket, roomId, username, roomState, onBack }) => {
    ...
    useEffect(() => {
        ...
        const handleIncomingCall = (data) => {
            // If I am the one who called (socket.id matches data.from), ignore.
        ...
        const handleVoiceStatus = ({ status, userId }) => {
            if (userId === socket.id) return; // Ignore my own status echo

            console.log("Voice Status Update:", status);
            setOpponentStatus(status);

            // AUTO-JOIN: If they accepted (came online) and I WAS calling, connect me immediately!
            if (status === 'online') {
                if (isCallingRef.current) {
                    console.log("🚀 Opponent Answered! Auto-connecting...");
                    setIsCalling(false); // Update State
            ...
            // SYNC DROP: If opponent goes offline, I should also hang up
            if (status === 'offline' && isMicOn) {
                // FIXED: We must emit 'offline' too (true), so the Initiator knows we dropped!
                // Otherwise they think we are still online and see "Join Call".
                endCall(true);
